...
function Header(){
    ...
    const [checkInLarge, setCheckInLarge] = useState(false);
    ...
    const handleClickWhere = (event) => {
        ...
        document.getElementById('search_expended').classList.remove('hidden');
    ...
    useEffect(() => {
        function handleClickOutside(event) {
            ...
            if (tripRef.current && !tripRef.current.contains(event.target)) {
                if (!document.getElementById('nav').contains(event.target)){
                    document.getElementById('search_expended').classList.add('hidden');
                    document.getElementById('trip_selector').classList.remove('bg-[#EBEBEB]');
                }
            }
        }
    ...
    useEffect(() => {
        document.getElementById('search_destination').addEventListener('input', function() {
            if (this.value.trim() === '') {
                ...
                <div>
                    <div id='trip_selector' ref={tripRef} className="relative flex items-center rounded-l-full rounded-r-full w-[850px] h-[66px] mb-4 shadow-2xl border border-gray-300">
                        <div id='where' ref={whereRef} onClick={handleClickWhere} className={`relative form-1 w-1/3 py-4 pl-8 h-full rounded-l-full rounded-r-full ${where ? 'bg-white' : 'hover:bg-gray-200'} `}>
                            ...
                            />
                            <div id='where_clicked' className='hidden absolute rounded-3xl bg-white left-0 shadow-lg -bottom-[510px] w-[478px] h-[498px]'>
                                <div className='py-5 px-10'>
                                    <div className='pt-8'>
                                        <p className='font-medium text-sm tracking-wide'>Search by Region</p>
                                    </div>
                                    <div className='flex flex-wrap mt-8 gap-3 gap-y-6'>
                                        <Region text="I'm flexible" imgSrc={region_1} handleClickCheckIn={handleClickCheckIn} />
                                        <Region text="Europe" imgSrc={region_2} handleClickCheckIn={handleClickCheckIn} />
                                        <Region text="Spain" imgSrc={region_3} handleClickCheckIn={handleClickCheckIn} />
                                        <Region text="Middle East" imgSrc={region_4} handleClickCheckIn={handleClickCheckIn}/>
                                        <Region text="Turkey" imgSrc={region_5} handleClickCheckIn={handleClickCheckIn} />
                                        <Region text="South East Asia" imgSrc={region_6} handleClickCheckIn={handleClickCheckIn} />
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div id="check_in" ref={CheckInRef} onClick={handleClickCheckIn} className={`py-4 h-full rounded-l-full rounded-r-full pl-6 ${stateExperiences ? 'w-1/3' : 'w-1/6'} ${checkIn ? 'bg-white' : 'hover:bg-gray-200' } ${checkIn ? '' : (stateExperiences ? 'form-2-big' : 'form-2-small')} `}>
                            <h6 className='text-xs cursor-pointer select-none'>{stateExperiences ? 'Date' : 'Check in'}</h6>
                            <p className='text-sm cursor-pointer select-none font-light opacity-60'>Add Dates</p>
                            <div id='check_in_clicked' className='hidden absolute rounded-3xl bg-white left-0 shadow-lg -bottom-[510px] w-full h-[555px] top-[76px]'>
                                <div className='py-5 px-10 h-full'>
                                    <div className='pt-8'>
                                        <p className='font-medium text-sm tracking-wide'>Custom Date Picker</p>
                                    </div>
                        ...
                        <div id="check_out" ref={CheckOutRef} onClick={handleClickCheckOut} className={`form-3 w-1/6 py-4 pl-6 h-full rounded-l-full rounded-r-full ${stateExperiences ? 'hidden' : ''} ${checkOut ? 'bg-white' : 'hover:bg-gray-200' } `}>
                            <h6 className='text-xs cursor-pointer select-none'>Check out</h6>
                            <p className='text-sm cursor-pointer select-none font-light opacity-60'>Add Dates</p>
                        </div>
                        <div id="who" ref={WhoRef} onClick={handleClickWho} className={`form-4 w-1/3 py-4 pl-8 h-full rounded-l-full rounded-r-full  ${who ? 'bg-white' : 'hover:bg-gray-200'}`}>
                            <h6 className='text-xs cursor-pointer select-none'>Who</h6>
                            <p className='text-sm cursor-pointer select-none font-light opacity-60'>Add Guests</p>
                            <div id='who_clicked' className={`absolute rounded-3xl bg-white right-0 shadow-lg -bottom-[407px] w-[406px] h-[395px] ${who ? '' : 'hidden'}`}>
                                <div className='py-5 px-10 flex-col flex justify-center'>
                                    <GuestSelector text="Adults" subtext="Ages 13 or above" />
                                    <GuestSelector text="Children" subtext="Ages 2-12" />
                                    <GuestSelector text="Infants" subtext="Under 2" />
                                    <GuestSelector text="Pets" subtext="Bringing a service animal?" />
                                </div>
                            </div>
                        </div>
                        <div className={`absolute left-1/3 h-[32px] w-[1px] bg-gray-300 border-form-1 ${checkIn ? 'hidden' : '' } ${where ? 'hidden' : '' } `} ></div>
                        <div className={`absolute right-1/3 h-[32px] w-[1px] bg-gray-300 border-form-3 ${stateExperiences ? (checkIn ? 'hidden' : '') : '' } ${checkOut ? 'hidden' : '' } ${who ? 'hidden' : '' } `} ></div>
                        <div id='border-2' className={`absolute right-1/2 h-[32px] w-[1px] bg-gray-300 border-form-2 ${stateExperiences ? 'hidden' : ''} ${checkIn ? 'hidden' : '' } ${checkOut ? 'hidden' : '' } `}></div>
                        <div className='absolute flex items-center right-2 bg-search-initial h-12 p-4 rounded-full transition-all duration-300 delay-150 transform'>
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" aria-hidden="true" role="presentation" focusable="false" className='block stroke-white fill-white h-4 w-4 stroke-[4] overflow-visible'><path fill="none" d="M13 24a11 11 0 1 0 0-22 11 11 0 0 0 0 22zm8-3 9 9"></path></svg>
                            <p id='search_expended' className='text-white font-medium text-[15px] px-2 hidden'>Search</p>
                        </div>
                    </div>
                </div>
            </div>
